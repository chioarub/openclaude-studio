...
Session replay timelines can contain user requests, tool input summaries, command arguments, error messages, and file paths. Studio applies the same secret-redaction patterns to every string field in the replay response (user content, tool input summaries, result previews, error messages, retry reasons, commands, and modified-file paths) and caps preview, summary, reason, and file-list lengths before returning data to the browser. Raw tool input objects are never returned — only the upstream `inputSummary` field is surfaced. Replay files are derived from validated session ids and already-authorized transcript roots; Studio never trusts paths embedded in replay JSON as read authorization, and rejects symlinks. Redaction remains best effort. Treat every replay step as potentially sensitive.
