...
When `OPENCLAUDE_CONFIG_DIR` and `CLAUDE_CONFIG_DIR` are both set and disagree, Studio emits a warning diagnostic through `/api/projects`. That message intentionally contains only the variable names and the resolution outcome — it never includes the configured path values, the home directory, or any local file contents.
