...
| `OPENCLAUDE_CONFIG_DIR` | `~/.openclaude` | OpenClaude config directory override (preferred). Studio reads the same root OpenClaude uses. A legacy `.openclaude/.config.json` is checked first; under an override, `.claude.json` is a legacy fallback if the modern `.openclaude.json` is missing. |
